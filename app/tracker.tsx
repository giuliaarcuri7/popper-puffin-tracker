...
export default function Tracker() {
  ...
  const [mapReady, setMapReady] = useState(false); const [index, setIndex] = useState(0); const [nextIndex, setNextIndex] = useState(1); const [progress, setProgress] = useState(0); const [letters, setLetters] = useState(1287342);
  ...
  const current = useMemo(() => { const a = destinations[index], b = destinations[nextIndex]; const longitudeDelta = ((b.lng - a.lng + 540) % 360) - 180; const longitude = ((a.lng + longitudeDelta * progress + 540) % 360) - 180; return { lat: a.lat + (b.lat - a.lat) * progress, lng: longitude, from: a.name, to: b.name }; }, [index, nextIndex, progress]);
  ...
  useEffect(() => {
    if (!mapRef.current || !process.env.NEXT_PUBLIC_MAPBOX_TOKEN) return; let active = true; let lightTimer: number | undefined;
    import("mapbox-gl").then(({ default: mapboxgl }) => { if (!active || !mapRef.current) return; mapboxgl.accessToken = process.env.NEXT_PUBLIC_MAPBOX_TOKEN!;
      ...
      map.on("load", () => { map.addSource("night-city-lights", { type: "geojson", data: nightLightsData(Date.now()) }); map.addLayer({ id: "night-city-light-glow", type: "circle", source: "night-city-lights", paint: { "circle-radius": ["interpolate", ["linear"], ["zoom"], 0, 3, 4, 6], "circle-color": "#ffd184", "circle-blur": .75, "circle-opacity": .88, "circle-stroke-width": 1, "circle-stroke-color": "#fff0c2", "circle-stroke-opacity": .55 } }); lightTimer = window.setInterval(updateNightLights, 5 * 60 * 1000); });
      ...
      mapMarker.current = new mapboxgl.Marker({ element: el }).setLngLat([current.lng, current.lat]).setPopup(new mapboxgl.Popup({ offset: 30 }).setHTML(`<strong>Popper Puffin</strong><br>${current.lat.toFixed(4)}, ${current.lng.toFixed(4)}`)).addTo(map);
