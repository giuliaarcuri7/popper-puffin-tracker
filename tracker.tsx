...
export default function Tracker() {
  ...
  useEffect(() => {
    ...
    import("mapbox-gl").then(({ default: mapboxgl }) => { if (!active || !mapRef.current) return; mapboxgl.accessToken = process.env.NEXT_PUBLIC_MAPBOX_TOKEN!;
      ...
      map.on("load", () => { focusOnPopper(); map.once("idle", focusOnPopper); focusTimer = window.setTimeout(focusOnPopper, 750); map.addSource("night-city-lights", { type: "geojson", data: nightLightsData(Date.now()) }); map.addLayer({ id: "night-city-light-glow", type: "circle", source: "night-city-lights", paint: { "circle-radius": ["interpolate", ["linear"], ["zoom"], 0, 3, 4, 6], "circle-color": "#ffd184", "circle-blur": .75, "circle-opacity": .88, "circle-stroke-width": 1, "circle-stroke-color": "#fff0c2", "circle-stroke-opacity": .55 } }); lightTimer = window.setInterval(updateNightLights, 5 * 60 * 1000); });
