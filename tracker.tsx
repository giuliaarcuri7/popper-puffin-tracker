...
export default function Tracker() {
  ...
  useEffect(() => { const timer = window.setInterval(() => { setProgress((p) => { if (p >= .98) { setIndex((i) => (i + 1) % route.length); setLetters((n) => n + 127); return 0; } return p + .01; }); setLetters((n) => n + 1); }, 1200); return () => clearInterval(timer); }, []);
  ...
  useEffect(() => {
    ...
    import("mapbox-gl").then(({ default: mapboxgl }) => { if (!active || !mapRef.current) return; mapboxgl.accessToken = process.env.NEXT_PUBLIC_MAPBOX_TOKEN!;
      ...
      map.addControl(new mapboxgl.NavigationControl({ showCompass: true }), "bottom-right"); map.on("style.load", () => map.setFog({ color: "#071529", "high-color": "#142f55", "horizon-blend": .15, "space-color": "#020813", "star-intensity": .7 }));
      const el = document.createElement("button"); el.className = "popper-marker"; el.title = "Popper’s exact location"; el.setAttribute("aria-label", "Popper’s exact location"); el.innerHTML = '<img src="/popper-marker.png" alt=""><span aria-hidden="true">🐧</span>';
  ...
  return <main className="app-shell">
    <header className="masthead"><div><p className="eyebrow">The North Pole Dispatch</p><h1>Popper Puffin Tracker</h1></div><div className="live"><span /> LIVE · CHRISTMAS EVE</div></header>
    <section className="dashboard" aria-label="Live Popper tracking dashboard">
      <aside className="portrait-card panel"><div className="portrait-frame"><img src="/popper-portrait.png" alt="Popper Puffin in his delivery cap" onError={(e) => e.currentTarget.style.display = "none"} /><span aria-hidden="true">🐧</span></div><p className="ribbon">CAPTAIN POPPER</p><p className="quote">“Every letter finds its way by starlight.”</p><dl className="mini-stats"><div><dt>Flight</dt><dd>PP-1224</dd></div><div><dt>Weather</dt><dd>Clear skies</dd></div></dl></aside>
      <section className="map-card panel" aria-label="Interactive globe"><div className="map-heading"><div><span className="label">CURRENT LEG</span><strong>{current.from} → {current.to}</strong></div><span className="map-status">SIGNAL STRONG</span></div><div className="globe-wrap"><div ref={mapRef} className="mapbox" aria-label="Mapbox globe" />{!mapReady && <div className="css-globe" role="img" aria-label="Animated nighttime globe preview"><div className="continents" /><div className="radar" /><button className="fallback-marker" title={`${current.lat.toFixed(4)}, ${current.lng.toFixed(4)}`} aria-label={`Popper at ${current.lat.toFixed(4)}, ${current.lng.toFixed(4)}`}>🐧</button></div>}<div className="coordinates" aria-live="polite">{current.lat.toFixed(4)}, {current.lng.toFixed(4)}</div></div><div className="instruments"><div><span>LATITUDE</span><strong>{coord(current.lat, "N", "S")}</strong></div><div className="compass" aria-label="Heading north east"><i>NE</i></div><div><span>LONGITUDE</span><strong>{coord(current.lng, "E", "W")}</strong></div><div><span>ALTITUDE</span><strong>{(28600 + Math.sin(progress * Math.PI) * 2400).toFixed(0)} FT</strong></div></div></section>
